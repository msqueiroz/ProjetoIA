...
# A pasta de documentos é opcional para a inicialização da aplicação.
# Ela concentra arquivos locais usados no POC (CSV, PDF etc.).
...
# Mantemos os nomes esperados pelo adaptador atual, mas só exibimos
# arquivos CSV que realmente existem em ProjetoIA/Documentos.
fontes_csv_conhecidas = [
    "dadosMult.csv",
    "dados_teste_qualidade.csv",
    "dados_heterogeneos.csv",
]

fontes_disponiveis = ["PI System - Simulado"]

for nome_csv in fontes_csv_conhecidas:
    if (PASTA_DOCUMENTOS / nome_csv).exists():
...
fonte_selecionada = st.selectbox(
    ...
    fontes_disponiveis,
    key="fonte_dados_principal"
)

if len(fontes_disponiveis) == 1:
    st.caption(
        "Nenhum CSV local foi encontrado em Documentos. "
        "A aplicação continua disponível com as demais fontes."
    )
...
# O adaptador de fontes existente ainda trabalha com nomes de arquivos
# relativos. Quando a fonte escolhida é CSV, executamos somente essa
# chamada dentro da pasta Documentos e restauramos imediatamente o
# diretório original. Assim, os demais módulos continuam independentes.
if fonte_selecionada.startswith("CSV - "):

    diretorio_original = Path.cwd()

    try:
        os.chdir(PASTA_DOCUMENTOS)
